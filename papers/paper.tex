...
However, Glossa in its current form is not intended to be industrial-level software. 
Instead, it is an investigation into creating a generalized compilation system that does not hardcore any rules. 
While Glossa is useful at doing \textit{some} automated translations, it is likely more useful for implementing new programming languages.
Because of the generalizations (discarding of hard-coded rules) that Glossa makes, changing language definitions is trivial compared to the difficulty of modifying the source of a compiler.

Glossa has accomplished far more than it originally set out to: Perhaps Glossa can be used as groundwork for an industrial-level compiler framework.
